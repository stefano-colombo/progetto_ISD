...
public class Consumer_x{
	...
	public void run(Message message) throws InterruptedException,IOException {
		...
		else if(new String(message.getBody()).equals("inizio_invio_file")) {
			grande=true;
			outputStream = new FileOutputStream(percorsoFileRicevuti+nomefile+"."+estensione);
			System.out.println(" \t\t\t\t\t\t\t\t[received]: ARRIVO di un GRANDE FILE'");
		}
		else if(!nomefile.equals("")){
			if(grande==true) {
				if(new String(message.getBody()).equals("fine")) {
					...
					try {
						outputStream = new FileOutputStream(percorsoFileRicevuti+nomefile+"."+estensione,true);
